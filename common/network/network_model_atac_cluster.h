...
class NetworkModelAtacCluster : public NetworkModel
{
   private:
      ...
      UInt32 m_electrical_mesh_link_bandwidth;
      ...
      QueueModel* m_receiver_hub_queue_model;
      ...
      UInt64 getHubQueueDelay(HubType hub_type, PacketType pkt_type, UInt64 pkt_time, UInt32 pkt_length);
      void outputHubSummary(ostream& out);

      UInt64 computeLatencySenderCoreToReceiverHub(const NetPacket& net_packet, core_id_t requester);
      UInt64 computeLatencyReceiverHubToReceiverCore(const NetPacket& net_packet, core_id_t requester);
   ...
   public:
      ...
      UInt64 computeHubQueueDelay(HubType hub_type, UInt64 pkt_time, UInt32 pkt_length);
};
