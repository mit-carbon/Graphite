                                            ...
                                            UInt32 access_num)
{
   ...
   if (cache_hit) //TODO: Verify this
   {
      if (mem_op_type == Core::WRITE)
         L1_cache_line_info.setCState(CacheState::MODIFIED);
      setCacheLineInfo(mem_component, address, &L1_cache_line_info);
   }
...
L1CacheCntlr::insertCacheLine(MemComponent::Type mem_component, IntPtr address, CacheState::Type cstate, Byte* fill_buf)
{
   ...
   if (eviction)
   {
      ...
      {
         LOG_ASSERT_ERROR(evicted_cstate == CacheState::SHARED || evicted_cstate == CacheState::EXCLUSIVE, "evicted_address(%#lx), evicted_cstate(%u)", //FH
...
L1CacheCntlr::handleMsgFromL2Cache(tile_id_t sender, ShmemMsg* shmem_msg)
{
   ...
   //case ShmemMsg::FLUSH_REQ:
      ...
      //break;
   case ShmemMsg::DOWNGRADE_REQ:
      processDowngradeReqFromL2Cache(sender, shmem_msg);
      break;
   ...
   if ((shmem_msg_type == ShmemMsg::EX_REP) || (shmem_msg_type == ShmemMsg::SH_REP) || (shmem_msg_type == ShmemMsg::SH_REP_EX) || (shmem_msg_type == ShmemMsg::UPGRADE_REP)) //FH
   {
...
L1CacheCntlr::processShRepFromL2Cache(tile_id_t sender, ShmemMsg* shmem_msg)
{
   ...
   // Insert Cache Line in L1-I/L1-D Cache
   ShmemMsg::Type shmem_msg_type = shmem_msg->getType();
   if(shmem_msg_type == ShmemMsg::SH_REP)
   {
      insertCacheLine(mem_component, address, CacheState::SHARED, data_buf);
   }
   else if(shmem_msg_type == ShmemMsg::SH_REP_EX) //FH
   {
      assert(mem_component == MemComponent::L1_DCACHE);
      insertCacheLine(mem_component, address, CacheState::EXCLUSIVE, data_buf);
   }
   else
      LOG_PRINT_ERROR("Unrecognized Msg Type(%u)",shmem_msg_type);
}
...
L1CacheCntlr::processInvReqFromL2Cache(tile_id_t sender, ShmemMsg* shmem_msg)
{
   ...
   {
      if(cstate == CacheState::MODIFIED) //FH
      {
         // MODIFIED -> INVALID
      ...
      else //cstate == CacheState::SHARED || cstate == CacheState::EXCLUSIVE
      {
         // Update Shared Mem perf counters for access to L1-D Cache
         _memory_manager->incrCurrTime(mem_component, CachePerfModel::ACCESS_TAGS);

         LOG_ASSERT_ERROR(cstate == CacheState::SHARED || cstate == CacheState::EXCLUSIVE, "cstate(%u)", cstate);
...
void
L1CacheCntlr::processDowngradeReqFromL2Cache(tile_id_t sender, ShmemMsg* shmem_msg)
{
   assert(!shmem_msg->isReplyExpected());
   ...
   {
      // EXCLUSIVE -> SHARED
      LOG_ASSERT_ERROR(cstate == CacheState::EXCLUSIVE || cstate == CacheState::MODIFIED, "cstate(%u)", cstate);
      ...
      else
      {
         ...
         ShmemMsg send_shmem_msg(ShmemMsg::DOWNGRADE_REP, MemComponent::L1_DCACHE, MemComponent::L2_CACHE,
               shmem_msg->getRequester(), shmem_msg->isReplyExpected(), address,
               shmem_msg->isModeled());
         _memory_manager->sendMsg(sender, send_shmem_msg);
      }
