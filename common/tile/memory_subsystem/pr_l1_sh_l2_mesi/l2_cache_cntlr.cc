...
L2CacheCntlr::allocateCacheLine(IntPtr address, ShL2CacheLineInfo* L2_cache_line_info)
{
   ...
   DirectoryEntry* directory_entry = DirectoryEntry::create(PR_L1_SH_L2_MESI,
...
L2CacheCntlr::handleMsgFromL1Cache(tile_id_t sender, ShmemMsg* shmem_msg)
{
   ...
   else if ( (shmem_msg_type == ShmemMsg::INV_REP) || (shmem_msg_type == ShmemMsg::FLUSH_REP) || (shmem_msg_type == ShmemMsg::WB_REP) || (shmem_msg_type == ShmemMsg::DOWNGRADE_REP))
   {
      ...
      else // (L2_cache_line_info.getCachingComponent() == shmem_msg->getSenderMemComponent())
      {
         ...
         case ShmemMsg::FLUSH_REP:
            ...
            break;
         case ShmemMsg::DOWNGRADE_REP:
            processDowngradeRepFromL1Cache(sender, shmem_msg, &L2_cache_line_info);
            break;
   ...
   case DirectoryState::EXCLUSIVE:
      {
         ...
         ShmemMsg shmem_msg(ShmemMsg::INV_REQ, MemComponent::L2_CACHE, MemComponent::L1_DCACHE,  //FH
      ...
      case DirectoryState::EXCLUSIVE:
         {
            // Flush the owner
            LOG_ASSERT_ERROR(directory_entry->getOwner() != INVALID_TILE_ID,
                             "Address(%#lx), State(EXCLUSIVE), owner(INVALID)", address);
            LOG_ASSERT_ERROR(L2_cache_line_info.getCachingComponent() == MemComponent::L1_DCACHE,
      ...
      case DirectoryState::SHARED:
         {
            ...
            {
               // Upgrade miss - shortcut - set state to EXCLUSIVE
               directory_entry->setOwner(requester);
               directory_entry->getDirectoryBlockInfo()->setDState(DirectoryState::EXCLUSIVE);
      ...
      case DirectoryState::EXCLUSIVE:
         {
            ...
            ShmemMsg shmem_msg(ShmemMsg::DOWNGRADE_REQ, MemComponent::L2_CACHE, MemComponent::L1_DCACHE,
                               requester, false, address,
                               msg_modeled);
            _memory_manager->sendMsg(directory_entry->getOwner(), shmem_msg);
         }
      ...
      case DirectoryState::UNCACHED:
         {
            ...
            DirectoryState::Type next_dstate = (requester_mem_component == MemComponent::L1_DCACHE) //FH
                                                ? DirectoryState::EXCLUSIVE : DirectoryState::SHARED;
            directory_entry->getDirectoryBlockInfo()->setDState(next_dstate);

            // Read the cache-line from the L2 cache and send it to L1
            if (next_dstate == DirectoryState::EXCLUSIVE) //FH
            {
               directory_entry->setOwner(requester);
               readCacheLineAndSendToL1Cache(ShmemMsg::SH_REP_EX, address, requester_mem_component, data_buf, requester, msg_modeled);
            }
            else
            {
               // Read the cache-line from the L2 cache and send it to L1
...
void
L2CacheCntlr::processDowngradeRepFromL1Cache(tile_id_t sender, const ShmemMsg* shmem_msg, ShL2CacheLineInfo* L2_cache_line_info)
{
   IntPtr address = shmem_msg->getAddress();
   ...
   case DirectoryState::EXCLUSIVE:
      {
         ...
         LOG_ASSERT_ERROR(!_L2_cache_req_queue.empty(address),
                          "Address(%#lx), DOWNGRADE_REP, req queue empty!!", address);
   ...
   case DirectoryState::UNCACHED:
   default:
      LOG_PRINT_ERROR("Address(%#llx), DOWNGRADE_REP, State(%u), num sharers(%u), owner(%i)",
                      address, curr_dstate, directory_entry->getNumSharers(), directory_entry->getOwner());
      break;
   }
}
