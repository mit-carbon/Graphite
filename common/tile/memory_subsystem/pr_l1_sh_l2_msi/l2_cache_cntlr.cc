...
L2CacheCntlr::allocateCacheLine(IntPtr address, ShL2CacheLineInfo* L2_cache_line_info)
{
   ...
   *L2_cache_line_info = ShL2CacheLineInfo(_L2_cache->getTag(address), directory_entry);
   ...
   if (eviction)
   {
      ...
      _L2_cache_req_queue_list.enqueue(evicted_address, nullify_req);
...
L2CacheCntlr::handleMsgFromL1Cache(tile_id_t sender, ShmemMsg* shmem_msg)
{
   ...
   if ( (shmem_msg_type == ShmemMsg::EX_REQ) || (shmem_msg_type == ShmemMsg::SH_REQ) )
   {
      ...
      _L2_cache_req_queue_list.enqueue(address, shmem_req);

      if (_L2_cache_req_queue_list.count(address) == 1)
      {
         // Process the request
   ...
   else if ( (shmem_msg_type == ShmemMsg::INV_REP) || (shmem_msg_type == ShmemMsg::FLUSH_REP) || (shmem_msg_type == ShmemMsg::WB_REP) )
   {
      ...
      if (!_L2_cache_req_queue_list.empty(address))
      {
         ShmemReq* shmem_req = _L2_cache_req_queue_list.front(address);
...
{
   HashMapQueue<IntPtr,ShmemReq*>::iterator it1 = _L2_cache_req_queue_list.begin();
   for ( ; it1 != _L2_cache_req_queue_list.end(); it1++)
   {
...
L2CacheCntlr::processNextReqFromL1Cache(IntPtr address)
{
   ...
   ShmemReq* completed_shmem_req = _L2_cache_req_queue_list.dequeue(address);
   ...
   case DirectoryState::MODIFIED:
      {
         ...
         LOG_ASSERT_ERROR(!_L2_cache_req_queue_list.empty(address),
