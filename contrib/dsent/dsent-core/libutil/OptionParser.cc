...
namespace LibUtil
{
    ...
    void OptionParser::parseArguments(int argc_, char** argv_, ostream& ost_)
    {
    ...
    void OptionParser::printOptions(ostream& ost_) const
    {
        ost_ << endl;
        ost_ << "Available options:" << endl;
        ost_ << "==================" << endl << endl;
        ...
        for(it = m_option_names_.begin(); it != m_option_names_.end(); ++it)
        {
            ...
            if(option_info->hasArg())
            {
                ost_ << " <" << option_info->getArgName() << ">";
            }
            ost_ << endl;

            ost_ << "    " << option_info->getDescription() << endl;
            if(option_info->hasArg() && option_info->hasDefaultArgValue())
            {
                ost_ << "    " << "Default: " << option_info->getDefaultArgValue() << endl;
            }
            ost_ << endl;
        }
        ost_ << "-help" << endl;
        ost_ << "    " << "Print this page" << endl;
        ost_ << endl;
        return;
    }
