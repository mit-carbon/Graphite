...
//  double aspect_ratio,ild_thickness, miller_value = 1.5, horiz_dielectric_constant, vert_dielectric_constant;
  ...
  // Owen
  else if (technology < 12 && technology > 10)
  {
    tech_lo = 11;
    tech_hi = 11;
  }
...
//      tech_hi = 16;
//    }
  else if (technology < 22 && technology > 11)
  {
    tech_lo = 22;
    tech_hi = 11;
  }
...
//    	I_g_on_n[1][100] = 4.51e-10;
    ...
    if (tech == 11)
    {
      SENSE_AMP_D = .03e-9; // s // Owen: TODO
      SENSE_AMP_P = 2.16e-15; // J // Owen: TODO
      vdd[0] = 0.6;
      Lphy[0] = 0.014;
      Lelec[0] = 0.014; // Owen: TODO
      t_ox[0] = 0.45e-3; // Owen: use EOTphy
      v_th[0] = 0.21835; // Owen: TODO
      c_ox[0] = 4.11e-14; // Owen: TODO
      mobility_eff[0] = 300 * (1e-2 * 1e6 * 1e-2 * 1e6); // Owen: use mu(nFET)
      Vdsat[0] = 5.09E-2; // Owen: TODO
      c_g_ideal[0] = 2.42e-15; // Owen: Hack (includes all C_g)
      c_fringe[0] = 0; // Owen: Hack
      c_junc[0] = 1.15e-15; // Owen: Hack (includes all C_d)
      I_on_n[0] = 1770.47e-6; // Owen: from Idsatn
      I_on_p[0] = 1610.72e-6; // Owen: from Idsatp
      nmos_effective_resistance_multiplier = 1.39; // Owen: 2*ln(2)
      n_to_p_eff_curr_drv_ratio[0] = 1.099; // Owen: INV_NMOS_EFF_CURRENT/ INV_PMOS_EFF_CURRENT
      gmp_to_gmn_multiplier[0] = 1.38; // Owen: TODO
      Rnchannelon[0] = nmos_effective_resistance_multiplier * vdd[0] / I_on_n[0];//ohm-micron // Owen: TODO need to double check
      Rpchannelon[0] = n_to_p_eff_curr_drv_ratio[0] * Rnchannelon[0];//ohm-micron
      long_channel_leakage_reduction[0] = 1.00; // Owen: set to 1 to that you don't get benifit from using long channel
      I_off_n[0][0] = 1.00e-7 / 100;
      I_off_n[0][10] = 1.21e-7 / 100;
      I_off_n[0][20] = 1.46e-7 / 100;
      I_off_n[0][30] = 1.78e-7 / 100;
      I_off_n[0][40] = 2.15e-7 / 100;
      I_off_n[0][50] = 2.61e-7 / 100;
      I_off_n[0][60] = 3.16e-7 / 100;
      I_off_n[0][70] = 3.83e-7 / 100;
      I_off_n[0][80] = 4.64e-7 / 100;
      I_off_n[0][90] = 5.62e-7 / 100;
      I_off_n[0][100] = 6.81e-7 / 100;

      I_g_on_n[0][0]  = 4.22e-8 / 100;//A/micron
      I_g_on_n[0][10] = 4.22e-8 / 100;
      I_g_on_n[0][20] = 4.22e-8 / 100;
      I_g_on_n[0][30] = 4.22e-8 / 100;
      I_g_on_n[0][40] = 4.22e-8 / 100;
      I_g_on_n[0][50] = 4.22e-8 / 100;
      I_g_on_n[0][60] = 4.22e-8 / 100;
      I_g_on_n[0][70] = 4.22e-8 / 100;
      I_g_on_n[0][80] = 4.22e-8 / 100;
      I_g_on_n[0][90] = 4.22e-8 / 100;
      I_g_on_n[0][100] = 4.22e-8 / 100;

      if (ram_cell_tech_type == lp_dram)
      {
        cerr << "This type of dram is not supported in this technology" << endl;
        exit(0);
      }
      else if (ram_cell_tech_type == comm_dram)
      {
...
//            		fringe_cap);
        ...
        {
          //Aggressive projections.
          wire_pitch[0][0] = 0.022; //2.5 * g_ip->F_sz_um;//local
          aspect_ratio[0][0] = 2.0;
          wire_width = wire_pitch[0][0] / 2;
          wire_thickness = aspect_ratio[0][0] * wire_width;
          wire_spacing = wire_pitch[0][0] - wire_width;
          barrier_thickness = 1.5e-3;
          dishing_thickness = 0;
          alpha_scatter = 1;
          wire_r_per_micron[0][0] = wire_resistance(BULK_CU_RESISTIVITY, wire_width,
            wire_thickness, barrier_thickness, dishing_thickness, alpha_scatter);
          ild_thickness[0][0] = 0.034;
          miller_value[0][0] = 1.5;
          horiz_dielectric_constant[0][0] = 2.3;
          vert_dielectric_constant[0][0] = 2.3;
          fringe_cap = 0.0;//0.115e-15;
          wire_c_per_micron[0][0] = wire_capacitance(wire_width, wire_thickness, wire_spacing,
            ild_thickness[0][0], miller_value[0][0], horiz_dielectric_constant[0][0], vert_dielectric_constant[0][0],
            fringe_cap);

          wire_pitch[0][1] = 0.034; //4 * g_ip->F_sz_um;//semi-global
          ...
          wire_r_per_micron[0][1] = wire_resistance(BULK_CU_RESISTIVITY, wire_width,
            wire_thickness, barrier_thickness, dishing_thickness, alpha_scatter);
          ild_thickness[0][1] = 0.0306;
          miller_value[0][1] = 1.5;
          horiz_dielectric_constant[0][1] = 2.3;
          vert_dielectric_constant[0][1] = 2.3;
          wire_c_per_micron[0][1] = wire_capacitance(wire_width, wire_thickness, wire_spacing,
            ild_thickness[0][1], miller_value[0][1], horiz_dielectric_constant[0][1], vert_dielectric_constant[0][1],
            fringe_cap);

          wire_pitch[0][2] = 0.088; //8 * g_ip->F_sz_um;//global
          aspect_ratio[0][2] = 2.34;
          wire_width = wire_pitch[0][2] / 2;
          wire_thickness = aspect_ratio[0][2] * wire_width;
          wire_spacing = wire_pitch[0][2] - wire_width;
          wire_r_per_micron[0][2] = wire_resistance(BULK_CU_RESISTIVITY, wire_width,
        		  wire_thickness, barrier_thickness, dishing_thickness, alpha_scatter);
          ild_thickness[0][2] = 0.642;
          miller_value[0][2] = 1.5;
          horiz_dielectric_constant[0][2] = 2.3;
          vert_dielectric_constant[0][2] = 2.3;
          wire_c_per_micron[0][2] = wire_capacitance(wire_width, wire_thickness, wire_spacing,
        		  ild_thickness[0][2], miller_value[0][2], horiz_dielectric_constant[0][2], vert_dielectric_constant[0][2],
        		  fringe_cap);

          //Conservative projections
          wire_pitch[1][0] = 0.022;//2.5 * g_ip->F_sz_um;
          aspect_ratio[1][0] = 2.0;
          wire_width = wire_pitch[1][0] / 2;
          wire_thickness = aspect_ratio[1][0] * wire_width;
          wire_spacing = wire_pitch[1][0] - wire_width;
          ...
          wire_pitch[1][1] = 0.034;//4 * g_ip->F_sz_um;
          wire_width = wire_pitch[1][1] / 2;
          aspect_ratio[1][1] = 2.0;
          wire_thickness = aspect_ratio[1][1] * wire_width;
          wire_spacing = wire_pitch[1][1] - wire_width;
          ...
          vert_dielectric_constant[1][1] = 3.9;
            ...
            wire_pitch[1][2] = 0.088;//8 * g_ip->F_sz_um;
            aspect_ratio[1][2] = 2.2;
            wire_width = wire_pitch[1][2] / 2;
            wire_thickness = aspect_ratio[1][2] * wire_width;
            wire_spacing = wire_pitch[1][2] - wire_width;
