...
//	  tag += int(XML->sys.domain_size + 5);
  ...
  interface_ip.out_w               = cachep.output_width * 8; // convert bytes to bits
...
void SharedCache::set_cache_param()
{
	if (cacheL==L2)
	{
      ...
      cachep.output_width  = XML->sys.L2[ithCache].L2_config[6];
	...
	else if (cacheL==L3)
	{
      ...
      cachep.output_width  = XML->sys.L3[ithCache].L3_config[6];
	...
	else if (cacheL==L1Directory)
		{
         ...
         cachep.output_width  = XML->sys.L1Directory[ithCache].Dir_config[6];
	...
	else if (cacheL==L2Directory)
		{
         ...
         cachep.output_width  = XML->sys.L2Directory[ithCache].Dir_config[6];
