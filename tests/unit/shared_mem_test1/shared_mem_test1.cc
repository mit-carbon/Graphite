...
int main (int argc, char *argv[])
{
   ...
   UInt32 num_misses;
   num_misses = (core_0->initiateMemoryAccess(MemComponent::L1_DCACHE, Tile::NONE, Tile::WRITE, address, (Byte*) &write_val_0, sizeof(write_val_0))).first;
   assert(num_misses == 1);

   // Tile 0 - Read out the value
   num_misses = (core_0->initiateMemoryAccess(MemComponent::L1_DCACHE, Tile::NONE, Tile::READ, address, (Byte*) &read_val_0, sizeof(read_val_0))).first;
   ...
   num_misses = (core_1->initiateMemoryAccess(MemComponent::L1_DCACHE, Tile::NONE, Tile::READ, address, (Byte*) &read_val_1, sizeof(read_val_1))).first;
   printf("Read Out(%u) from address(0x%x)\n", read_val_1, address);
   assert(num_misses == 1);
   ...
   printf("Writing(%u) into address(0x%x)\n", write_val_1, address);
   num_misses = (core_1->initiateMemoryAccess(MemComponent::L1_DCACHE, Tile::NONE, Tile::WRITE, address, (Byte*) &write_val_1, sizeof(write_val_1))).first;
   assert(num_misses == 1);
